...
var Logger = /** @class */ (function () {
    ...
    Logger.prototype.log = function (_class) {
        ...
        { // Pipes content to file based on pipe
            ...
            for (var _c = 0, pipes_1 = pipes; _c < pipes_1.length; _c++) {
                var pipe = pipes_1[_c];
                var stream = fs_1.createWriteStream(pipe.path, {
                    flags: 'a+'
                });
            ...
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        now = Date.now();
                        if (name)
                            this.log("LOGGER", "Benchmarking", termx_1.highlight(name));
                        return [4 /*yield*/, promise];
                    case 1:
                        result = _a.sent();
                        duration = Date.now() - now;
                        this.log("LOGGER", termx_1.highlight(name), "finished after", termx_1.cold(duration + "ms"));
